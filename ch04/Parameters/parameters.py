...
def init_normal(m):
    if type(m) == nn.Linear:
        nn.init.normal_(m.weight, mean=0, std=0.01) # normal_/zeros_(apply_to, mean, std)
...
def my_init(m):
    if type(m) == nn.Linear:
        ...
        m.weight.data *= m.weight.data.abs() >= 5 # 把小于 5 的绝对值元素全部置为0，只保留绝对值大于等于 5 的元素（用bool做到的）
